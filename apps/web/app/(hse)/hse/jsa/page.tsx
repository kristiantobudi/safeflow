...
import { JsaUpsertModal } from '@/components/hse/jsa-upsert-modal';
import Link from 'next/link';

export default function JsaPage() {
  const [search, setSearch] = useState('');
  const [isCreateOpen, setIsCreateOpen] = useState(false);
  const { data: jsaData, isLoading, error } = useJsaList();
  ...
  const stats = useMemo(() => {
    return {
      ...
      pending: jsaList.filter((jsa: any) => jsa.approvalStatus === 'PENDING')
        .length,
      submitted: jsaList.filter((jsa: any) => jsa.approvalStatus === 'SUBMITTED')
        .length,
      approved: jsaList.filter((jsa: any) => jsa.approvalStatus === 'APPROVED')
        .length,
      rejected: jsaList.filter((jsa: any) => jsa.approvalStatus === 'REJECTED')
        .length,
    };
  }, [jsaList]);
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }: any) => (
        <span className="font-medium text-foreground">
          {row.original.jenisKegiatan || '-'}
        </span>
      ),
    },
    {
      accessorKey: 'lokasiKegiatan',
      header: 'Lokasi Kegiatan',
      cell: ({ row }: any) => (
        <span className="font-medium text-muted-foreground">
          {row.original.lokasiKegiatan || '-'}
        </span>
      ),
    },
    {
      ...
      cell: ({ row }: any) => (
        <div className="flex items-center gap-2 text-muted-foreground">
          ...
          <span className="text-sm">
            {row.original.tanggalDibuat
              ? new Date(row.original.tanggalDibuat).toLocaleDateString('id-ID')
              : '-'}
          </span>
        </div>
      ),
    },
    {
      id: 'actions',
      cell: ({ row }: any) => (
        <Link href={`/hse/jsa/${row.original.id}`}>
  ...
  const handleRowClick = (row: any) => {
    ...
    window.location.href = `/hse/jsa/${row.original.id}`;
  };
  ...
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto animate-in fade-in duration-500">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground">
              Manajemen analisis keselamatan kerja dan pengajuan JSA.
            </p>
          </div>
          <Button
        ...
        <Card className="border-destructive/50 bg-destructive/5">
          <CardContent className="pt-6">
            <div className="flex items-center gap-3 text-destructive">
              ...
              <div>
                <p className="font-semibold">Gagal memuat data JSA</p>
      ...
      <div className="relative w-full sm:max-w-md">
        ...
        <Input
          placeholder="Cari jenis kegiatan atau lokasi..."
      ...
      <JsaUpsertModal open={isCreateOpen} onOpenChange={setIsCreateOpen} />
    </div>
  );
}